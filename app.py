...
if not DATABASE_URL:
    sqlite_path = os.environ.get("SQLITE_PATH", "echomirror.db")
...
def send_daily_reminder_emails(dry_run: bool = False) -> dict:
    ...
    with smtplib.SMTP(smtp_host, smtp_port, timeout=30) as server:
        ...
        for recipient in recipients:
            ...
            message["Subject"] = "Echo Mirror reminder: log today's check-in"
            message["From"] = smtp_from
            message["To"] = recipient
            message.set_content(
                "Quick reminder from Echo Mirror. "
...
def dashboard():
    ...
    if current:
        today = date.today()
        total_days = (current.end_date - current.start_date).days + 1
        elapsed_days = (today - current.start_date).days + 1
        elapsed_days = max(0, min(elapsed_days, total_days))
        days_left = max((current.end_date - today).days, 0)
        progress = {
    ...
    return render_template(
        "dashboard.html", current=current, summary=summary, progress=progress
    )
...
def results():
    ...
    chart_checkins = list(reversed(checkins))

    return render_template(
        "results.html",
        user_experiment=latest,
        summary=summary,
        checkins=checkins,
        chart_checkins=chart_checkins,
    )
...
def export_checkins_csv():
    ...
    filename = f"echo_mirror_checkins_{date.today().isoformat()}.csv"
    return Response(
        output.getvalue(),
        mimetype="text/csv",
...
def export_metrics_csv():
    ...
    filename = f"echo_mirror_metrics_{date.today().isoformat()}.csv"
    return Response(
        output.getvalue(),
        mimetype="text/csv",
        headers={"Content-Disposition": f"attachment; filename={filename}"},
